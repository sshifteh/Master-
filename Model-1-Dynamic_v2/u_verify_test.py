...
def main(w, N):  
	...
	plot(u_c, mesh, interactive = True, title = 'Exact Velocity Field')
	plot(U,mesh, interactive = True, title = 'Experimental Velocity Field')
        #interactive()
	#u_e = ()

	#********************* noe nytt 
	#u_e = interpolate(u_c, Vspace)
	#bcs.apply(u_e.vector())
	#u_error = errornorm(u_e, U, norm_type='l2', degree_rise=0, mesh=mesh)
	# utgjorde ingen forskjell 

	u_error = errornorm(u_c, U, norm_type='l2', degree_rise=3, mesh=mesh)

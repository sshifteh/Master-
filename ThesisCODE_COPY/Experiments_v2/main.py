...
from indicator_tuned import indicator_tuned
from indicator_old import indicator_old
from indicator_new import tau_xy, interface, indicator

from K_update_capped import K_update, K_cap
...
x0 = 0.5           # the center point of the vessel 0.0 
R = 0.20            # the radius of the vessel 0.20 # 0.10 
viz = True          # vizualization parameter
speed = 2.0         # factor multiplied with the Poiseuille flow to alternatie the speed  
h = 1./N            # the mesh size  
x1 = 0.1            # the area to be cut at the inlet 
mu = 1              # the dynamic viscosity 
dpdy = 2.1          # the pressure gradient  
epsilon = 2         # the growth or shrinking factor 
alg = 't'           # algorithm 'o'=old;  'n'=new; t=tuned


time_start = 0     # time start
time_stop = 3      # time stop
...
# The surface
phi_I_square = Expression('( x[0] > (x0 - R - DOLFIN_EPS) && x[0] < (x0 + R + DOLFIN_EPS)) ? 0.0 : 1.0', x0 = x0, R = R )
K  = interpolate(phi_I_square, P) # alternativt DG0 
...
interesting_domain_square = Expression('(x[0] > x1 & x[1] > x1 & x[0] < 1- x1 & x[1] < 1-x1) ? 1.0 : 0.0', x1 =x1)
interesting_domain_interpolate = interpolate(interesting_domain_square, DG1)          
